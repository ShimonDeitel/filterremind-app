import XCTest
@testable import AirFilterReminder

@MainActor
final class AirFilterReminderTests: XCTestCase {
    func testAddIncreasesCount() {
        let store = Store()
        let before = store.entries.count
        store.add(FilterEntry())
    ...
    func testCannotAddMoreWhenAtLimitAndNotPro() {
        ...
        store.entries = (0..<Store.freeLimit).map { _ in FilterEntry() }
